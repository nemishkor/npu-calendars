<?php
class Model_Auditories extends Model
{
	
	function __construct($registry){
		$table_name = 'auditories';
	...
	function create(){
		...
		$query = "INSERT INTO `{$this->table_name}` VALUES(NULL, '{$item['published']}', '0', '{$item['created_by']}', '{$item['name']}', '{$item['institute']}')";
		$result = $this->db->query($query);
		if($result){
	...
	function save(){
		$item = $this->get_item_from_form();
		$query = "UPDATE {$this->table_name} SET name='{$item['name']}', published='{$item['published']}', institute='{$item['institute']}' WHERE id='{$item['id']}'";
		$result = $this->db->query($query);
		if($result){
			return $item['id'];
		} else
	...
	function get_item_from_form(){
		...
		if(isset($_GET['name']) && $_GET['name']){
			$item = array('published'=>$_GET['published'], 'name'=>$_GET['name'], 'created_by'=>$user['id'], 'institute'=>$_GET['institute']);
